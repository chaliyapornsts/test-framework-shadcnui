    ...
    return (
        <div>
            <div className="mt-6 flex justify-end lg:justify-center">
                ...
                <div className='lg:hidden mx-10 cursor-pointer'>
                    ...
                    >
                        ...
                        <div>
                            ...
                            <div className='flex max-h-screen overflow-y-scroll'>
                                <Button
                                    className={`justify-start text-gray-600 font-bold text-sm hover:text-orange-500 px-5 py-1 items-start bg-white hover:bg-white ${selectedArtist === "" ? 'text-orange-500' : ''}`}
                                    onClick={() => setSelectedArtist("")}
                                >
                                    All
                                </Button>
                                {AZAsciis.map(ascii => (
                                    <Button
                                        key={ascii}
                                        className={`justify-start text-gray-600 font-bold text-sm hover:text-orange-500 px-5 py-1 items-start bg-white hover:bg-white ${selectedArtist === String.fromCharCode(ascii) ? 'text-orange-500' : ''}`}
                                        onClick={() => setSelectedArtist(String.fromCharCode(ascii))}
                                    >
                                        {String.fromCharCode(ascii)}
                                    </Button>
                        ...
                        <div className="flex flex-wrap">
                            {groupedData[firstChar].map(item => (
                                ...
                                <div className="h-0 pb-[100%] relative">
                                    <img className="mx-auto object-cover w-full h-full absolute top-0 bottom-0 left-0 right-0 rounded-full artist-img5" src={item.imgSrc} alt={item.artist} />
                                </div>
...
export default NationalArtists5;