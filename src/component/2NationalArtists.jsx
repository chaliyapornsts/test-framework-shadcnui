                    ...
                    <div key={firstChar} className="w-full">
                        ...
                        <div className="flex flex-wrap">
                            {groupedData[firstChar].map(item => (
                                <Link key={item.id} className="p-5 w-1/2 md:w-1/3 lg:w-1/4 cursor-pointer artist-link" to={`/profile/${item.id}`}>
                                    <div className="h-0 pb-[100%] relative">
                                        <img className="mx-auto object-cover w-full h-full absolute top-0 bottom-0 left-0 right-0 rounded-full artist-img2" src={item.imgSrc} alt={item.artist} />
                                    </div>
...
export default NationalArtists2;
